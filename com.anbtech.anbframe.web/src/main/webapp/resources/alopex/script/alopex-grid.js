...
//		tag.child = _escapeHTML(tag.child);
	...
	_renderValue.plugin["text"] = function(render, value, data, mapping) {
		...
		if (typeof render.attr == "object") {	
			for ( var prop in render.attr) {
				if (prop == "style") {
					...
					rendered.push(render.attr[prop], aligntext,'" ');
				} else {
					rendered.push(prop, '="', render.attr[prop], '" ');
				}
			}
		...
		if(!styleoccured) {
			rendered.push('style="width:100%;',aligntext,'"');
		}
		var irule = null;
...
//					fixedColgroup.children('col').map(function(idx,elem){if(idx>self.state.fixupto) return elem;}).remove();
		...
		if(self.option.enableHeaderGroupResizing && self.option.headerGroup && self.option.headerGroup.length) {
			...
			var widthmap = self._calcCellWidth();
			...
			if(prevValue !== value) {
				...
				if(self.option.mergeEditingImmediately) {
					if(!data._state._beforeEditing) {
						data._state._beforeEditing = AlopexGrid.trimData(data);
					}
...
//								$body.children().eq(-1).attr('data-alopexgrid-dataindex'),'(to-1)')
	...
	function getRowHeight(self, $row) {
		...
		return _max(height, $row.children().height())+(self.option.rowHeightCompensate||0);
	}
	/**
	...
	AlopexGrid.prototype._calcCellWidth = function(){
		...
		var rowstring = self._rowRender(self.state.emptyData, null, null,{"styleclass":cname,"disableRowspan":true,"catchError":true});
		var $trow = $(rowstring);
	...
	AlopexGrid.prototype._cellRender = function(data, mapping, option) {
		...
		if (mapping.numberingColumn) {
			...
			for(var mi in multiList) {
				...
				var multirow = {tag:"tr",
					attr:{
						"styleclass":"row bodyrow multirow",
				...
				if(multirowOption) {
					if(multirowOption.styleclass) {
						...
						multirow.attr['styleclass'] += ($.isFunction(s) ? (' '+s(data, multiMapping, multirowOption)) : s) || '';
					}
			...
			if(renderer === "editable" && mapping.editable) {
				...
				renderer = $.extend({}, mapping.editable);
			}
					...
					if(sorting || resizing) {
						...
						if (sorting && mapping.key) {
							...
							if(option.hideSortingHandle!==true) {
								var handle = createSortingHandle();
...
//			$row.replaceWith(_convertAlopex.call(self, newrow));
	...
	AlopexGrid.prototype._refreshRenderedArray = function() {
		var self = this;
...
//			}
	...
	AlopexGrid.prototype.rowSelect = function(query, selected, e) {
		...
		for(var i=0,l=datalist.length;i<l;i++) {
			...
			self.state.dataCompositor(actualdata);
		}
		if (option.rowClickSelect === "only" || option.rowSingleSelect) {
